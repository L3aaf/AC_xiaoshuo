# _*_coding:utf-8
# @Time      :2023/11/26 0:25
# @Author    :DQ
# @FileName: sortvisit_link.py
import re
...
class sortvisit_link:
    ...
    def get_links(self):
        ...
        # 使用 XPath 获取排序链接和标题
        sort_links = tree.xpath(r'/html/body/div[3]/div/div[/]/ul//a[not(ancestor::p)]/@href')
        sort_title = tree.xpath(r'/html/body/div[3]/div/div[/]/ul//a/text()')
    ...
    def get_content(self):
        titles_list = self.get_title_list()
        ua = Acxiaoshuo.tool.ua.Ua()
        ...
        for index, title_list in enumerate(titles_list, start=2):
            html_content = ua.read_html('sort/', title_list)
            tree = html.fromstring(html_content)
